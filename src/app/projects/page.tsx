...
export default async function ProjectsPage({ searchParams }: { searchParams?: Promise<{ topic?: string | string[] }> }) {
  ...
  const initialSelectedTopic = rawTopic === 'all' ? null : rawTopic ?? undefined;
    ...
    <div className="min-h-screen py-16">
      <div className="mx-auto max-w-7xl px-6 mt-16 md:mt-24">
        ...
        <Suspense fallback={<ProjectsLoading />}>
          <ProjectsGrid initialProjects={projects} initialSelectedTopic={initialSelectedTopic} />
        </Suspense>
      </div>
    </div>
  );
